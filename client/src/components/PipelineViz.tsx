/**
 * PipelineViz v6.1 — Grid-aligned version.
 * Strict CSS Grid for all sections. No flex guesswork.
 */
...
/* ── Stepper node ───────────────────────────────────────── */
function StepNode({ ok, active, icon }: { ok: boolean; active: boolean; icon: React.ReactNode }) {
  return (
    <div className="relative flex items-center justify-center">
      ...
      `}>
        {ok ? <Check className="h-4 w-4" /> : active ? <Loader2 className="h-4 w-4 animate-spin" /> : icon}
      </div>
...
/* ── Connector line ─────────────────────────────────────── */
function StepLine({ ok, active }: { ok: boolean; active: boolean }) {
  return (
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-5 sm:p-6">
        ...
        {/* ═══ Phase stepper — 7-column grid: node-line-node-line-node-line-node ═══ */}
        <div className="grid grid-cols-[36px_1fr_36px_1fr_36px_1fr_36px] items-start mb-6">
          {steps.map((s, i) => {
            ...
            return (
              <div key={s.id} className="contents">
                ...
                <div className="flex flex-col items-center gap-1">
                  <StepNode ok={ok} active={active} icon={s.icon} />
                  ...
                  {(ok || active) && s.sub && (
                    <span className="text-[8px] text-muted-foreground text-center whitespace-nowrap">{s.sub}</span>
                  )}
                </div>
                {/* Line column (skip after last node) */}
                {i < steps.length - 1 && (
                  <div className="px-2">
                    <StepLine ok={ok} active={active} />
                  </div>
                )}
              </div>
            );
        ...
        {/* ═══ Engine grid — equal columns ═══ */}
        <div className="mb-5">
          <div className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.08em] mb-2.5">Движки перевода</div>
          <div
            ...
            {engines.map(eng => {
              ...
              return (
                <div key={eng} className={`
                  flex flex-col items-center justify-start gap-1.5 py-2.5 rounded-xl transition-all duration-300 min-h-[76px]
                ...
                `}>
                  ...
                  {/* Fixed-height slot for latency/error so cards stay same height */}
        ...
        {/* ═══ Latency bar chart — CSS Grid: label | bar | value ═══ */}
